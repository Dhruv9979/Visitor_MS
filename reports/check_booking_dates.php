<?php 
    ...
    {  
        ...
        try 
        {
            ...
            WHERE DateVisit >= '". date("Y-m-d",strtotime($_POST['startDate']))."' 
            AND DateVisit <= '". date("Y-m-d",strtotime($_POST['endDate']))."' 
            ORDER BY GroupName";
        }
        else{
            $sql = "SELECT tab_visitorgroups.*, tab_organizers.ContactName FROM tab_visitorgroups 
            ...
            WHERE DateVisit >= '". date("Y-m-d",strtotime($_POST['startDate']))."' 
            AND DateVisit <= '". date("Y-m-d",strtotime($_POST['endDate']))."' 
            AND tab_visitorgroups.GroupName = '".$_POST["check_GroupName"]."'
            ORDER BY GroupName";
        }
        ...
        if(!empty($result))
        {
            $js_code = '<script>' . console.log("<?php $statement->errorCode();?>") . '</script>';
            $js_code .= '<script>' . console.log("<?php $statement->errorInfo();?>") . '</script>';
    ...
    if(isset($_POST["booking_dates_pdf"]))  
    {  
        ...
        ';  
        $content .= fetch_data_dates();  
        $content .= '</table>';  
        $obj_pdf->writeHTML($content);  
        ob_end_clean();
        $obj_pdf->Output('check-booking-dates.pdf', 'I');    
    }  
?>